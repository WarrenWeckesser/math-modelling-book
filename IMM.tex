...
\vspace{0.25in}
{\small Copyright \copyright ~2005 Warren Weckesser}}
%
...
\chapter{Differential Equations}\index{differential equation}
...
\item Sketch $\frac{dy}{dt}$ as a function of $y$.\\
\item In one set of axes, sketch several solutions $y(t)$. Include the equilibrium solutions,
...
\begin{enumerate}
\item $\ds y' = -y/(t^2+1)$, \hspace{0.25cm} $y(0)=5$.
\item $\ds y' = -\left(2-e^{-t}\right)y$, \hspace{0.25cm} $y(0)=3$.
\item $\ds y' = (t+1)y^2$, \hspace{0.25cm} $y(0)=y_0$, where $y_0$ is
a constant.
\end{enumerate}
\end{exercise}
\end{exercises}
...
\begin{enumerate}
\item $\ds y' = 2y+t$, \hspace{0.25cm} $y(0)=10$
\item $\ds y' = -\frac{y}{2} + \sin(t)$, \hspace{0.25cm} $y(0)=0$
\end{enumerate}
\end{exercise}
\end{exercises}
...
that
\[
  \lim_{t\rightarrow\infty}\BX(t) = \BZ.
\]
All trajectories approach $\BZ$ asymptotically.
...
\begin{figure}
\centerline{%
\includegraphics[angle=270,width=4.5in]{PeriodicDrugDosePlot.eps}
}
...
\end{equation}
A \emph{basis} for the solution set of~\eqref{eqn:linearsys}
...
\begin{figure}
\centerline{%
\includegraphics[angle=270,width=4.5in]{PeriodicDrugDosePlotWithXinf.eps}
}
