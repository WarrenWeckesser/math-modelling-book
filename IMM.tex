...
\end{figure}
Since $\frac{dp}{dt} = 0$ when $p = \frac{b}{r}$, we have an
equilibrium solution $p(t) = \frac{b}{r}$.

For $p < \frac{b}{r}$, $\frac{dp}{dt} > 0$, and for
$p > \frac{b}{r}$, $\frac{dp}{dt} < 0$.
So if $p(t) < \frac{b}{r}$, $p(t)$ is an increasing function of
$t$.  As $p$ increases, $\frac{dp}{dt}$ decreases; the closer
$p$ is to $\frac{b}{r}$, the closer $\frac{dp}{dt}$
is to zero.
This suggests that $p(t)$ will approach $\frac{b}{r}$ asymptotically
from below as $t\rightarrow\infty$.
A similar argument shows that if $p(t) > \frac{b}{r}$,
$p(t)$ will approach $\frac{b}{r}$ asymptoticall from above.

The plot on the right in Figure~\ref{fig:auton_example_plots}
shows an example of a solution, for which $0 < p(0) < \frac{b}{r}$.
\end{example}
