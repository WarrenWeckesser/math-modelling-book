...
\end{split}
\end{equation}
\subsection*{Example: The SI Components of the SIR Model.}
...
\begin{figure}
\fbox{%
\input{matlab/eigenvalues.output}
}
...
the SIR model \eqref{eqn:SIR}.  The output generated by this code is shown
...
\begin{figure}
\centerline{\includegraphics[width=4.5in]{matlab/SIRscript.plot.eps}}
...
Figure \ref{fig:scilabeigenvalueslisting} shows an example
of using the \texttt{spec} functio; the output is shown
in Figure \ref{fig:scilabeigenvaluesoutput}.
The computation shows that the eigenvalues of the 
matrix are $1$ and the complex conjugate pair
$0.2 \pm  0.2449490i$
(approximately).
...
\begin{figure}
\fbox{%
\input{scilab/eigenvalues.sce.listing}
}
\caption{Listing of \texttt{eigenvalues.sci}.}
\label{fig:scilabeigenvalueslisting}
\end{figure}

\begin{figure}
\fbox{%
\input{scilab/eigenvalues.output.listing}
}
\caption{The output that results when
\texttt{eigenvalues.sce} is run in Scilab.}
